...
// ~1609px at 1920x951, ~1207px at 1440x900). expect(locator).toHaveScreenshot()
...
//
// This component also uses useParallax (speeds 1.5/1.05) on its
// .image-left/.image-right pictures -- unlike break's/quote's parallax
// targets, this one runs on real image content, and driving scroll via
// scrollIntoViewIfNeeded()'s underlying wheel-adjacent physics previously
// produced hundreds of px of run-to-run displacement in those pictures'
// positions. The deterministic scrollTo(target, false, position) below has no
// physics involved at all, which resolves that as a side effect.
  ...
  for (const viewport of [{ width: 1920, height: 951 }, { width: 1440, height: 900 }, { width: 390, height: 844 }]) {
    test(`matches at ${viewport.width}x${viewport.height}`, async ({ page }) => {
      ...
      if (viewport.width >= 1024) {
        ...
        await jumpTo(page, '.mask_imgtext');
      } else {
