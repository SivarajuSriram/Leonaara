...
const wrapperCls = 'h-[60rem]! mx-[calc((-1*var(--grid-gap))/2)]! max-lg:h-[30rem]! max-md:h-[29.7rem]! max-lg:ml-[calc(var(--grid-gap)/2.22)]! max-lg:mr-0!';
...
const slideImgCls = '[&_img]:h-full [&_img]:w-auto max-lg:[&_img]:max-w-[100vw] max-md:[&_img]:max-w-[99vw]';
const slidePictureCls = 'h-full! w-auto! max-lg:max-w-[100vw]! max-md:max-w-[99vw]!';
...
// gated: blurs every slide behind a "Download Floor Plan" button (see GatedPlan.tsx) and
// disables the click-to-open lightbox, which would otherwise reveal the plan -- opted into
// by PlansTabs.tsx for the floor-plan categories.
export function ImgSlider({ section, alwaysCentered = false, loop = true, gated = false }: { section: ImgSliderSection; alwaysCentered?: boolean; loop?: boolean; gated?: boolean }) {
  ...
  return (
    <div className={cls} {...{ uid: `c${section.id}` }}>
      ...
      >
        {images.map((img, i) => (
          <SwiperSlide key={i} className={slideCls} onClick={gated ? undefined : () => setLightboxIndex(i)}>
            {gated ? (
              <GatedPlan className="h-full w-fit">
