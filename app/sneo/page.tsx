...
export const metadata: Metadata = metadataFor(sneo);
...
export default function SneoPage() {
  const [, pagefilter] = sneo.columns.colPos0;
  if (pagefilter.type !== 'mask_pagefilter') throw new Error('expected pagefilter');
  return (
    <main>
      <BodyClass pageId={sneo.id} layout="layout-0" />
      <Hero section={experiencesHero} />
      <FilterShell id={pagefilter.id} appearance={pagefilter.appearance} variant="pagefilter" items={items} nodes={nodes} activeIndex={4} metaByIndex={metaByIndex} />
    </main>
  );
}
