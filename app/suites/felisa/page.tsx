...
export const metadata: Metadata = metadataFor(felisa);
...
export default function FelisaPage() {
  const [, rooms] = felisa.columns.colPos0;
  if (rooms.type !== 'mask_rooms') throw new Error('expected rooms');
  return (
    <main>
      <BodyClass pageId={felisa.id} layout="layout-0" />
      <Hero section={suitesHero} />
      <FilterShell id={rooms.id} appearance={rooms.appearance} variant="rooms" items={items} nodes={nodes} activeIndex={2} metaByIndex={metaByIndex} />
    </main>
  );
}
