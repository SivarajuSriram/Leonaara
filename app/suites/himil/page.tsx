...
export const metadata: Metadata = metadataFor(himil);
...
export default function HimilPage() {
  const [, rooms] = himil.columns.colPos0;
  if (rooms.type !== 'mask_rooms') throw new Error('expected rooms');
  return (
    <main>
      <BodyClass pageId={himil.id} layout="layout-0" />
      <Hero section={suitesHero} />
      <FilterShell id={rooms.id} appearance={rooms.appearance} variant="rooms" items={items} nodes={nodes} activeIndex={3} metaByIndex={metaByIndex} />
    </main>
  );
}
