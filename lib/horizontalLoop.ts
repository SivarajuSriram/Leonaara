...
// Internal-only member (the public interface above stays as documented in the task brief);
// closestIndex is used by current()/toIndex()/the drag handlers but isn't part of the public surface.
type InternalLoopTimeline = LoopTimeline & { closestIndex: (setCurrent?: boolean) => number };
